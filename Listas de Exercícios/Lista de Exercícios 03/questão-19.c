/*
19. Faça uma função busca que efetuará a busca de um elemento passado por
parâmetro, em uma lista duplamente encadeada. Sua função deverá retornar em
que posição está na lista, caso ache o elemento e -1 em caso contrário.
*/
...
int inserir(Lista *l, int v) {
  ...
  if(!pAnterior) { // inserir na primeira posição
    novo_elemento->proximo = p;
    l->inicio = novo_elemento;

  } else if (p == NULL) { // inserir na ultima posição
